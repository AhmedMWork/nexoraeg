...
Deno.serve(async (req) => {
  ...
  try {
    if (body.action === 'update') { const s = body.settings || {}; const patch = { brand_name: s.storeName, logo: s.logo, favicon: s.favicon, shipping_fee: s.shippingFee, free_shipping_threshold: s.freeShippingThreshold, whatsapp_number: s.whatsappNumber, support_email: s.supportEmail, currency: s.currency, cod_enabled: s.codEnabled, maintenance_mode: s.maintenanceMode, default_language: s.defaultLanguage, default_theme: s.defaultTheme, social_links: s.socialLinks, payment_settings: s.paymentSettings, launch_settings: s.launchSettings, meta_pixel_enabled: s.metaPixelEnabled, meta_pixel_id: s.metaPixelId, return_policy_text: s.returnPolicyText, shipping_policy_text: s.shippingPolicyText, seo: s.seo }; Object.keys(patch).forEach((k) => patch[k] === undefined && delete patch[k]); const { error } = await supabase.from('site_settings').update(patch).eq('id', 'main'); if (error) throw error; return json({ ok: true }); }
    if (body.action === 'storefront-get') { const { data, error } = await supabase.from('site_settings').select('home_collection_tiles').eq('id', 'main').maybeSingle(); if (error) throw error; return json({ homeCollectionTiles: data?.home_collection_tiles || [] }); }
    if (body.action === 'storefront-update') { const tiles = Array.isArray(body.homeCollectionTiles) ? body.homeCollectionTiles.slice(0, 5) : []; const { error } = await supabase.from('site_settings').update({ home_collection_tiles: tiles, updated_at: new Date().toISOString() }).eq('id', 'main'); if (error) throw error; return json({ ok: true, homeCollectionTiles: tiles }); }
