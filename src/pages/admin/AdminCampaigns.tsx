...
const platforms = [
  { value: 'facebook', label: 'Facebook Ad', source: 'facebook', medium: 'paid_social', example: 'Paid Feed / Reels campaign' },
  { value: 'instagram', label: 'Instagram Ad', source: 'instagram', medium: 'paid_social', example: 'Paid Story / Reels campaign' },
  { value: 'google', label: 'Google Search', source: 'google', medium: 'paid_search', example: 'Paid search ad' },
  { value: 'organic_instagram', label: 'Organic Instagram', source: 'instagram', medium: 'organic_social', example: 'Bio link / organic story' },
  { value: 'direct_whatsapp', label: 'WhatsApp Share', source: 'whatsapp', medium: 'direct_message', example: 'Manual WhatsApp message' },
];
...
export default function AdminCampaigns() {
  ...
  const create = async () => {
    ...
    toast.success('Campaign link saved. Copy it for the ad, bio, or story.');
    await load();
  };

  const copy = async (value: string) => {
    await navigator.clipboard.writeText(value);
    toast.success('Copied. Paste it into the ad link.');
  };

  const applyExample = (example: typeof quickExamples[number]) => {
    setForm(example);
    toast.success('Example loaded. Edit if needed, then copy or save.');
  };
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm leading-6 text-[#BCAEA0]">Create tracking links to identify visits from Facebook, Instagram, Google, WhatsApp, or any other source.</p>
        </div>
      ...
      <div className="studio-card p-5">
        ...
        <div className="grid gap-3 md:grid-cols-4">
          {[
            ['1. Choose the platform', 'Choose Facebook, Instagram, Google, or organic/social sharing.'],
            ['2. Name the campaign', 'Use a clear name such as june_drop or last_pieces to group visits in reports.'],
            ['3. Add content', 'Use it to compare different ads.'],
            ['4. Copy the link', 'Paste the final URL in the ad, bio, story, or WhatsApp message.'],
      ...
      <div className="grid gap-4 lg:grid-cols-[0.85fr_1.15fr]">
        <div className="studio-card p-5">
          <h2 className="mb-4 flex items-center gap-2 text-sm font-semibold text-[#FFF0E1]"><Plus className="h-4 w-4 text-[#D2B48C]" />Link Builder</h2>
        ...
        <div className="space-y-4">
          ...
          <div className="studio-card p-5">
            ...
            <div className="space-y-2">
              ...
              }) : <p className="text-sm leading-7 text-[#BCAEA0]">No campaign reports yet. Use the links in ads, then review reports.</p>}
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="studio-card overflow-hidden">
        ...
        <div className="overflow-x-auto"><table className="w-full min-w-[980px] text-left"><thead className="bg-[#17110F]"><tr>{['Name','Source / Medium','Campaign','Content','Landing','Final URL'].map((h) => <th key={h} className="p-4 text-[10px] uppercase tracking-[0.18em] text-[#BCAEA0]">{h}</th>)}</tr></thead><tbody>{isLoading ? <tr><td colSpan={6} className="p-8 text-center text-sm text-[#BCAEA0]">Loading links...</td></tr> : links.length ? links.map((link) => <tr key={link.id || link.finalUrl} className="border-t border-[#332923]/70"><td className="p-4 text-xs font-semibold text-[#FFF0E1]">{link.name}</td><td className="p-4 text-xs text-[#D2B48C]">{link.source}<br /><span className="text-[#BCAEA0]">{link.medium}</span></td><td className="p-4 text-xs text-[#BCAEA0]">{link.campaign}</td><td className="p-4 text-xs text-[#BCAEA0]">{link.content || '—'}</td><td className="p-4 text-xs text-[#BCAEA0]">{link.landingPage}</td><td className="p-4 text-xs"><button onClick={() => copy(link.finalUrl)} className="max-w-[360px] truncate text-left text-[#D2B48C]">{link.finalUrl}</button></td></tr>) : <tr><td colSpan={6} className="p-8 text-center text-sm text-[#BCAEA0]">No saved links yet.</td></tr>}</tbody></table></div>
      </div>
    </div>
  );
}
