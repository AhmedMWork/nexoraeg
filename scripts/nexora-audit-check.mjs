...
for (const file of files) {
  ...
  if (file !== join(root, 'scripts', 'nexora-audit-check.mjs') && text.includes('Sent / متابعة 2')) failures.push(`${file}: old follow-up placeholder`);
...
if (failures.length) {
  console.error('NEXORA audit failed:');
  for (const failure of failures) console.error(`- ${failure}`);
  process.exit(1);
}

console.log(`NEXORA audit passed (${files.length} files scanned).`);
